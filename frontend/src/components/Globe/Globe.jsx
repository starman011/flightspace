...
export const Globe = forwardRef(function Globe({ aircraft, selectedId, onAircraftClick, onAirportClick, onViewportChange, trackingId, solarData, padMarker, onInteract, onPlanetClick, onSkyObjectClick, onMoonSiteClick, neoData, onZoomChange, mobilePanel, tooltipDimmed, onScaleReady, showWeather }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    ...
    enableAR: async () => {
      const ar = int.current.arController
      if (!ar) return 'err: tracker not ready'
      try { await ar.enable(); return 'ok' }
      catch (e) {
        const m = (e && e.message) || String(e) || 'unknown error'
        console.warn('[AR] enable failed:', e?.name, m)
        return 'err:' + (e?.name ? e.name + ': ' : '') + m
      }
    },
    disableAR: () => int.current.arController?.disable(),
  ...
  useEffect(() => {
    ...
    int.current.arController = arController   // bridge to the imperative AR handles
    ...
    const airportLabelEls = AIRPORTS.map(a => {
      ...
      div.style.cssText = `position:absolute;display:none;transform:translate(-50%,-100%) translateY(-5px);padding:3px 6px 3px 5px;background:rgba(4,9,14,0.88);border:1px solid rgba(163,230,53,0.45);border-radius:4px;color:rgba(163,230,53,0.85);font:700 8px/1.3 var(--font-mono,monospace);white-space:nowrap;pointer-events:auto;cursor:pointer;backdrop-filter:blur(6px);box-shadow:0 1px 8px rgba(0,0,0,0.6),0 0 6px rgba(163,230,53,0.1)`
      div.title = `${a.name} — ${a.city}`
      div.addEventListener('click', () => { int.current.onAirportClick?.(a.iata) })
      div.addEventListener('mouseenter', () => { div.style.background = 'rgba(163,230,53,0.12)'; div.style.borderColor = 'rgba(163,230,53,0.9)'; div.style.color = '#a3e635'; div.style.boxShadow = '0 2px 12px rgba(0,0,0,0.7),0 0 14px rgba(163,230,53,0.25)' })
      div.addEventListener('mouseleave', () => { div.style.background = 'rgba(4,9,14,0.88)'; div.style.borderColor = 'rgba(163,230,53,0.45)'; div.style.color = 'rgba(163,230,53,0.85)'; div.style.boxShadow = '0 1px 8px rgba(0,0,0,0.6),0 0 6px rgba(163,230,53,0.1)' })

...
// FindAirportInDirection finds the nearest airport in a given heading direction
// within maxKm and coneDeg, using the full OpenFlights airport database (7700+).
func FindAirportInDirection(lat, lon, heading, maxKm, coneDeg float64) *AirportCoords {
	var best *AirportCoords
	bestDist := math.Inf(1)
	for _, a := range AirportByIATA {
...
// FindNearestAirport finds the closest airport to a position within maxKm,
// using the full OpenFlights airport database.
func FindNearestAirport(lat, lon, maxKm float64) *AirportCoords {
	var best *AirportCoords
	bestDist := math.Inf(1)
	for _, a := range AirportByIATA {
		d := haversine(lat, lon, a.Lat, a.Lon)
